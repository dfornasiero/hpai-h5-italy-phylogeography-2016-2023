post_tree_extraction = function(post_tre, mostRecentSamplingDatum)
	{
		post_tab = matrix(nrow=dim(post_tre$edge)[1], ncol=7)
		studyArea = matrix(nrow=dim(post_tre$edge)[1], ncol=2) 
		colnames(post_tab) = c("node1","node2","length","startLon","startLat","endLon","endLat")
		colnames(studyArea) = c("startStudyArea","endStudyArea")
		...
		for (i in 1:length(post_tre$annotations))
			{
				...
				studyArea[i,"endStudyArea"] = annotations$studyArea
			}
		for (i in 1:length(post_tre$annotations))
			{
		...
		startYear = matrix(post_tab[,"startNodeL"]-maxEndL)
	 	...
	 	post_tab = cbind(post_tab,startYear,endYear, studyArea)
